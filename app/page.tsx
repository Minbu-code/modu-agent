...
type InvestigationRoute = "principal" | "review";
...
const closureChecks = [
  { title: "처리 결과 확인", detail: "필요한 업무가 모두 완료되었는지 확인합니다." },
  { title: "종결 요약 확인", detail: "비식별 정보로 처리 결과가 기록되어 있는지 확인합니다." },
  { title: "담당교사 최종 확인", detail: "종결 전 기록과 문서를 담당교사가 최종 확인합니다." },
];

const principalClosureConditions = [
  { title: "사실관계가 명확히 확인됨", detail: "확인된 사실과 추가 확인이 필요한 내용을 구분했습니다." },
  { title: "학교 지침상 학교장 종결 요건 확인", detail: "학교 지침과 담당 부서의 적용 요건을 확인했습니다." },
  { title: "피해학생 보호 및 재발 방지 계획 확인", detail: "필요한 보호조치와 재발 방지 계획을 검토했습니다." },
  { title: "관련 학생·보호자 안내 및 의사 확인", detail: "안내 내용과 확인 결과를 기록할 준비가 되었습니다." },
];
...
function CaseList({ cases, selectedId, onSelect, onDelete }: { cases: CaseItem[]; selectedId: string; onSelect: (item: CaseItem) => void; onDelete?: (item: CaseItem) => void }) {
  return <div className="case-list">{cases.map((item) => <div className="case-row-wrap" key={item.id}><button className={selectedId === item.id ? "case-row selected" : "case-row"} onClick={() => onSelect(item)}><div className="case-number">{item.number}</div><div className="case-copy"><strong>{item.title}</strong><span>{item.date} · {item.location}</span></div><div className="case-stage"><span>{item.stage}</span><div className="mini-progress"><i style={{ width: `${item.progress}%` }} /></div></div><div className="case-due"><span className={statusClass(item.status)}>{item.status}</span><small>{item.due}</small></div><span className="chevron">›</span></button>{onDelete && <button className="case-delete" aria-label={`${item.number} 사안 삭제`} onClick={() => onDelete(item)}>삭제</button>}</div>)}</div>;
}

function Workflow({ selected, tasks, onToggle, onAskAdvance, onAskClosure, closureConditionDone, onToggleClosureCondition, onChooseInvestigationRoute }: { selected: CaseItem; tasks: Task[]; onToggle: (index: number) => void; onAskAdvance: () => void; onAskClosure: () => void; closureConditionDone: boolean[]; onToggleClosureCondition: (index: number) => void; onChooseInvestigationRoute: (route: InvestigationRoute) => void }) {
  const completed = tasks.filter((task) => task.done).length;
  const progress = tasks.length ? Math.round((completed / tasks.length) * 100) : 0;
  const isClosureStage = selected.stage === stages[stages.length - 1];
  const isInvestigationComplete = selected.stage === "조사" && tasks.length > 0 && completed === tasks.length;
  const conditionsMet = closureConditionDone.length === principalClosureConditions.length && closureConditionDone.every(Boolean);
  return <section className="panel workflow-panel"><div className="panel-heading"><div><p className="section-kicker">SELECTED CASE · {selected.number}</p><h2>{selected.title}</h2></div><span className={statusClass(selected.status)}>{selected.status}</span></div><div className="case-summary"><div><span>발생 일시</span><strong>{selected.date}</strong></div><div><span>발생 장소</span><strong>{selected.location}</strong></div><div><span>현재 단계</span><strong>{selected.stage}</strong></div></div><div className="stage-track">{stages.map((stage, index) => <div key={stage} className={stages.indexOf(selected.stage) >= index ? "stage active" : "stage"}><span>{index + 1}</span><small>{stage}</small></div>)}</div>{isClosureStage ? <><div className="task-header"><div><h3>종결 전 최종 확인</h3><span>아래 내용을 확인한 후 사안을 종결 처리합니다.</span></div><div className="progress-ring closure-ring"><span>확인</span></div></div><div className="closure-check-list">{closureChecks.map((check) => <div className="closure-check" key={check.title}><span className="checkmark">✓</span><span><strong>{check.title}</strong><small>{check.detail}</small></span></div>)}</div>{selected.status !== "종결" ? <button className="advance-button" onClick={onAskClosure}>✓ 확인 후 사안 종결 처리</button> : <div className="closure-complete">✓ 사안 종결 처리가 완료되었습니다.</div>}</> : isInvestigationComplete ? <div className="route-panel"><div className="route-heading"><div><h3>조사 완료 · 후속 처리 경로 선택</h3><p>학교장 종결 요건을 모두 확인한 경우에만 학교장 종결을 선택할 수 있습니다.</p></div><span className={conditionsMet ? "route-status route-status-ready" : "route-status"}>{conditionsMet ? "요건 충족" : "요건 확인 필요"}</span></div><div className="closure-condition-list">{principalClosureConditions.map((condition, index) => <label className={closureConditionDone[index] ? "closure-condition done" : "closure-condition"} key={condition.title}><input type="checkbox" checked={Boolean(closureConditionDone[index])} onChange={() => onToggleClosureCondition(index)} /><span className="checkmark">✓</span><span><strong>{condition.title}</strong><small>{condition.detail}</small></span></label>)}</div>{conditionsMet ? <div className="route-actions"><button className="route-button principal-route" onClick={() => onChooseInvestigationRoute("principal")}>학교장 종결 처리</button><button className="route-button review-route" onClick={() => onChooseInvestigationRoute("review")}>심의 준비로 이동</button></div> : <><p className="route-warning">학교장 종결 조건이 모두 충족되지 않아 심의 준비 단계로 이동해야 합니다.</p><button className="advance-button" onClick={() => onChooseInvestigationRoute("review")}>심의 준비 단계로 이동</button></>}</div> : <><div className="task-header"><div><h3>{selected.stage} 단계 체크리스트</h3><span>{completed}/{tasks.length}개 완료 · 진행률 {progress}%</span></div><div className="progress-ring" style={{ background: `conic-gradient(var(--blue) ${progress}%, #e9edf4 0)` }}><span>{progress}%</span></div></div><div className="task-list">{tasks.map((task, index) => <label key={`${task.id ?? task.title}-${index}`} className={task.done ? "task done" : "task"}><input type="checkbox" checked={task.done} onChange={() => onToggle(index)} /><span className="checkmark">✓</span><span><strong>{task.title}</strong><small>{task.detail}</small></span></label>)}</div>{completed === tasks.length && <button className="advance-button" onClick={onAskAdvance}>{selected.stage === stages[stages.length - 2] ? "✓ 조치 체크리스트 완료 · 종결 확인 단계로 이동" : "✓ 체크리스트 완료 · 다음 단계로 이동"}</button>}</>}</section>;
}
...
export default function Home() {
  ...
  const [closureConditionDone, setClosureConditionDone] = useState<boolean[]>(() => principalClosureConditions.map(() => false));
  ...
  useEffect(() => {
    setClosureConditionDone(principalClosureConditions.map(() => false));
  }, [selectedId]);
  ...
  async function toggleTask(index: number) {
    ...
    const isReopening = selected.stage === closingStage && !nextDone;
    ...
    if (isReopening) {
      const restoredTasks = emptyTasks(previousStage);
      setTasks(restoredTasks);
      setCases((current) => current.map((item) => item.id === selected.id ? { ...item, stage: previousStage, progress: 0, status: "진행 중" } : item));
      if (session) {
        await supabase.from("case_tasks").update({ status: "대기", completed_at: null, updated_at: new Date().toISOString() }).eq("case_id", selected.id).eq("stage", previousStage).eq("user_id", session.user.id);
        await supabase.from("cases").update({ current_stage: previousStage, progress_rate: 0, status: "진행 중", updated_at: new Date().toISOString() }).eq("id", selected.id).eq("user_id", session.user.id);
        await loadTasks(selected.id, previousStage);
      }
      return;
    }
    const reopenedStatus = !nextDone && selected.status === "검토 대기" ? "진행 중" : selected.status;
    setCases((current) => current.map((item) => item.id === selected.id ? { ...item, progress: nextProgress, status: reopenedStatus } : item));
    if (session) await supabase.from("cases").update({ progress_rate: nextProgress, status: reopenedStatus, updated_at: new Date().toISOString() }).eq("id", selected.id).eq("user_id", session.user.id);
  }

  function toggleClosureCondition(index: number) { setClosureConditionDone((current) => current.map((done, itemIndex) => itemIndex === index ? !done : done)); }

  async function moveToStage(nextStage: Stage) {
    if (!nextStage || nextStage === selected.stage) return;
    const nextTasks = emptyTasks(nextStage);
    const nextStatus: CaseStatus = nextStage === stages[stages.length - 1] ? "검토 대기" : "진행 중";
    setCases((current) => current.map((item) => item.id === selected.id ? { ...item, stage: nextStage, progress: 0, status: nextStatus } : item));
    setTasks(nextTasks); setConfirmAdvance(false);
    if (session) { await supabase.from("cases").update({ current_stage: nextStage, progress_rate: 0, status: nextStatus, updated_at: new Date().toISOString() }).eq("id", selected.id).eq("user_id", session.user.id); if (nextTasks.length) await supabase.from("case_tasks").insert(nextTasks.map((task) => ({ case_id: selected.id, user_id: session.user.id, stage: nextStage, task_title: task.title, task_description: task.detail, status: "대기" }))); await loadTasks(selected.id, nextStage); }
  }

  async function advanceStage() {
    const currentIndex = stages.indexOf(selected.stage); if (currentIndex < 0 || currentIndex === stages.length - 1) return;
    await moveToStage(stages[currentIndex + 1]);
  }

  async function chooseInvestigationRoute(route: InvestigationRoute) {
    const allConditionsMet = closureConditionDone.every(Boolean);
    if (route === "principal" && !allConditionsMet) { await moveToStage("심의 준비"); return; }
    await moveToStage(route === "principal" ? "종결" : "심의 준비");
  }

  async function finalizeClosure() {
    setCases((current) => current.map((item) => item.id === selected.id ? { ...item, status: "종결", progress: 100, due: "완료" } : item));
  ...
  function requestDeleteCase(item: CaseItem) { setDeleteTarget(item); setDeleteError(""); }

  async function deleteCase() {
    if (!deleteTarget) return;
    if (session) {
      const { error } = await supabase.from("cases").delete().eq("id", deleteTarget.id).eq("user_id", session.user.id);
      if (error) { setDeleteError(error.message); return; }
    }
    const remaining = cases.filter((item) => item.id !== deleteTarget.id);
    setCases(remaining); setDeleteTarget(null);
    if (selectedId === deleteTarget.id) {
      if (remaining.length) { setSelectedId(remaining[0].id); await loadTasks(remaining[0].id, remaining[0].stage); }
      else { setSelectedId(""); setTasks([]); }
    }
  }
