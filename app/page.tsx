...
function Sidebar({ activeNav, onNavigate, session, onSignOut }: { activeNav: string; onNavigate: (item: string) => void; session: Session; onSignOut: () => void }) {
  ...
  return <aside className="sidebar">
    <div className="brand"><div className="brand-mark">학</div><div><strong>모두의 학폭비서</strong><span>학교 업무 지원 도구</span></div></div>
...
export default function Home() {
  ...
  if (dataLoading && !session) return <main className="auth-screen"><div className="auth-card loading-card"><div className="auth-logo"><div className="brand-mark">학</div><div><strong>모두의 학폭비서</strong><span>학교 업무 지원 도구</span></div></div><p className="section-kicker">SECURE WORKSPACE</p><h1>안전한 업무 공간을 준비하고 있어요</h1><p className="auth-copy">로그인 상태와 사안 정보를 확인하는 중입니다.</p><div className="loading-line" /></div></main>;
  if (!session) return <main className="auth-screen"><div className="auth-card"><div className="auth-logo"><div className="brand-mark">학</div><div><strong>모두의 학폭비서</strong><span>학교 업무 지원 도구</span></div></div><p className="section-kicker">SECURE WORKSPACE</p><h1>{authMode === "signin" ? "업무를 이어서 시작하세요" : "담당교사 계정 만들기"}</h1><p className="auth-copy">사안 정보는 로그인한 사용자 본인만 조회할 수 있습니다.</p><form onSubmit={handleAuth}><label>이메일<input type="email" value={authEmail} onChange={(event) => setAuthEmail(event.target.value)} placeholder="teacher@school.kr" required /></label><label>비밀번호<input type="password" value={authPassword} onChange={(event) => setAuthPassword(event.target.value)} placeholder="6자 이상 입력" minLength={6} required /></label>{authError && <div className="auth-error">{authError}</div>}<button className="primary-button auth-submit" type="submit" disabled={authBusy}>{authBusy ? "처리 중..." : authMode === "signin" ? "로그인" : "계정 만들기"}</button></form><button className="auth-switch" onClick={() => { setAuthMode(authMode === "signin" ? "signup" : "signin"); setAuthError(""); }}>{authMode === "signin" ? "처음 사용하시나요? 계정 만들기" : "이미 계정이 있나요? 로그인"}</button><small className="auth-foot">개인정보는 비식별 원칙에 따라 입력해 주세요.</small></div></main>;
  ...
  else pageContent = <><div className="content-grid"><div className="main-column"><section className="metric-grid"><div className="metric-card"><div className="metric-label"><span className="metric-dot blue" />진행 중인 사안</div><strong>{cases.filter((item) => item.status !== "종결").length}<small>건</small></strong><span className="metric-foot">전체 사안 기준</span></div><div className="metric-card"><div className="metric-label"><span className="metric-dot orange" />오늘 확인할 업무</div><strong>3<small>건</small></strong><span className="metric-foot">마감이 가까운 순서</span></div><div className="metric-card"><div className="metric-label"><span className="metric-dot red" />마감 임박 업무</div><strong>1<small>건</small></strong><span className="metric-foot danger-text">오늘 안에 확인 필요</span></div><div className="metric-card"><div className="metric-label"><span className="metric-dot green" />종결 대기 사안</div><strong>2<small>건</small></strong><span className="metric-foot">마지막 확인 단계</span></div></section><section className="panel case-panel"><div className="panel-heading"><div><p className="section-kicker">MY CASES</p><h2>최근 사안</h2></div><button className="text-button" onClick={() => setActiveNav("사안 관리")}>전체 보기 →</button></div><CaseList cases={cases} selectedId={selected.id} onSelect={selectCase} /></section><Workflow selected={selected} tasks={tasks} onToggle={toggleTask} onAskAdvance={() => setConfirmAdvance(true)} /></div><aside className="right-column">{renderAiPanel(true)}<section className="panel caution-panel"><div className="panel-heading"><div><p className="section-kicker">CHECK BEFORE YOU USE</p><h3>담당교사 확인사항</h3></div><span className="caution-icon">!</span></div><ul><li>확인된 사실과 추정 내용을 구분했나요?</li><li>학생을 특정할 수 있는 정보가 없나요?</li><li>학교 지침과 실제 사실관계를 확인했나요?</li></ul></section></aside></div></>;
